...
> **Linux/Waybar port — work in progress.**
> This is a Linux port of [CodeZeno/Claude-Code-Usage-Monitor](https://github.com/CodeZeno/Claude-Code-Usage-Monitor),
> a native Windows taskbar widget by Craig Constable. The goal is a Waybar
> integration (initially targeting Arch + Hyprland/Wayland) that reuses the
> upstream polling/credential/usage logic and emits Waybar-friendly JSON.
> The Linux frontend is not built yet — the documentation below still
> describes the upstream **Windows** application. See [Credits](#credits).
...
This is a Linux/Waybar port derived from the original Windows application:

- **Claude Code Usage Monitor** by Craig Constable —
  <https://github.com/CodeZeno/Claude-Code-Usage-Monitor>

The original work is MIT licensed. Substantial portions of the polling,
credential, localization, and usage-formatting logic originate from that
project and are reused here under that license. See [`LICENSE`](LICENSE) and
